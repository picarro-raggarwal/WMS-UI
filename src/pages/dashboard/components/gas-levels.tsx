...
const GasLevelsCard = () => {
  const { data: gasTanksData, isLoading: isLoadingGasTankData, isError } = useGetGasTanksQuery();
  ...
  if (isError) {
    return (
      <Card className="mt-4 p-6">
        <CardTitle className="mb-3 pb-3 border-b-2 dark:text-white">Cylinder Fill Levels</CardTitle>
  ...
  return (
    <Card className="mt-4 p-6">
      ...
      {!gasTanksData ||
        (gasTanksData.gas_tanks.length === 0 && (
          <EmptyStateInfo
                    ...
                    <TooltipTrigger asChild>
                      <div className="cursor-help">
                        ...
                        {gasTank.tank_status !== "ok" ? (
                          <span className={getStatusStyle(gasTank.tank_status).className}>
                            {getStatusStyle(gasTank.tank_status).icon}
                            {formatLabel(gasTank.tank_status)}
                          </span>
                        ) : (
                          <div className="text-gray-500 dark:text-gray-400 text-xs">Full</div>
                        )}
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
              ...
              <div className="pt-3 border-gray-100 dark:border-neutral-700 border-t">
                <div className="mb-1 text-gray-600 dark:text-gray-400 text-xs">
                  ...
                  <span className="mb-1 ml-1 font-medium text-gray-900 dark:text-white">
                    {format(new Date(gasTank?.last_replaced_time), DATE_FORMAT, {})}
                  </span>
                </div>
