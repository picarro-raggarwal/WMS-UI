...
import type { MetricRange, SystemMetric } from "../data/systemMetrics.slice";
import {
  METRIC_BEHAVIORS,
  useGetSystemMetricsQuery
} from "../data/systemMetrics.slice";
...
function RangeIndicator({
  ...
  const getSegmentColor = (
    ...
    const shouldBeRed = behavior.higherIsBetter
      ? status === "low"
      : status === "high";
        ...
        {segments.length > 0 ? (
          segments.map((segment, index) => (
            <div key={index} className="flex items-center gap-2">
              ...
              />
              <span>
                {segment.label}: {segment.min.toFixed(1)}-
                {segment.max.toFixed(1)}
                <span className="ml-1 text-neutral-300 text-sm">{unit}</span>
              </span>
            </div>
          ))
        ) : (
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        {segments.length > 0 && (
          <Tooltip delayDuration={0}>
            ...
            <TooltipContent
              side="bottom"
              className="shadow-xl p-4 border-none rounded-xl min-w-md"
            >
              {tooltipContent}
            </TooltipContent>
          </Tooltip>
...
function MetricCard({ metric }: { metric: SystemMetric }) {
  const segments = metric.range
    ? getSegmentsFromRange(metric.range, metric.name)
    : [];
  const isMfcMetric = metric.name.toLowerCase().includes("mfc");

  return (
    <div className="flex flex-col gap-2 bg-white dark:bg-neutral-800 shadow-card shadow-sm p-4 rounded-xl ring-1 ring-neutral-500/10 -50">
      <div>
        <div className="flex justify-between items-center">
          <dt className="font-medium text-neutral-800 dark:text-neutral-100 text-base tracking-tight">
            {formatLabel(metric.name)}
          </dt>
        </div>
        <div>
          <p className="mt-1 font-semibold dark:text-white text-lg tracking-tight">
            {metric.value === null ? (
              "-"
            ) : (
              <NumberFlow
            ...
            <span className="ml-1 text-neutral-600 text-sm">{metric.unit}</span>
          </p>
          {isMfcMetric &&
            metric.set_point !== null &&
            metric.set_point !== undefined && (
...
export function MetricsCards() {
  ...
  if (isMetricsDataError || hasStableError) {
    return (
      <div className="relative">
        <div className="top-0 left-0 z-10 absolute flex flex-col justify-center items-center gap-2 bg-gradient-to-b from-transparent to-white rounded-xl w-full h-full /50 lur-sm">
        ...
        <dl className="gap-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
          {[...Array(6)].map((_, index) => (
  ...
  const filteredMetrics = metricsData?.system_metrics?.filter(
    (metric) => metric.name !== "ups"
  );

  if (isMetricsLoading || !filteredMetrics) {
    return (
      <div className="relative">
        <dl className="gap-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
          {[...Array(6)].map((_, index) => (
  ...
  return (
    <>
      <dl className="gap-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 min-h-[200px]">
