        ...
        Object.entries(latestData).forEach(([metric, data]) => {
          if (data.timestamps.length > 0 && data.values.length > 0) {
            ...
            if (updatedData[metric]) {
              ...
              updatedData[metric] = [
                ...updatedData[metric].filter(
                  (point) => point.timestamp > oneDayAgoMs
                ),
                newPoint
              ];
            } else {
    ...
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-neutral-500 text-center">
          ...
          <p className="text-sm">
            Please select metrics to display real-time trends
          </p>
        </div>
      </div>
    );
            ...
            <div className="bg-white dark:bg-neutral-800 shadow-card p-6 dark:border dark:border-neutral-700/20 rounded-xl">
              <div className="space-y-4">
                <DataReviewChartHeader
                  ...
                  showWarning={
                    thresholdVisibility[metric]?.showWarning || false
                  }
                  ...
                  currentValue={
                    chartData[metric][chartData[metric].length - 1]?.value
                  }
                        ...
                        metricStats[metric].thresholds
                          ? {
                              warning: {
                                ...
                                color: "#f97316", // orange
                                visible:
                                  thresholdVisibility[metric]?.showWarning ||
                                  false
                              },
                              alarm: {
                                ...
                                color: "#ef4444", // red
                                visible:
                                  thresholdVisibility[metric]?.showAlarm ||
                                  false
                              }
                            }
                          : null
